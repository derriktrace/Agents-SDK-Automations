...
initial_analysis_agent = Agent[PhishingAnalysisContext](
    ...
    You are the Initial Analysis Agent. Your role is to perform preliminary checks and prepare for detailed analysis.
    1. Receive the EML filename.
    2. **Execute the `file_check_and_hash_tool`**. This tool:
        a. Verifies the file exists and appears to be an EML.
        b. Calculates MD5/SHA256 hashes.
        c. Gets the VirusTotal reputation summary for the EML file's hash.
    3. Review the tool's output. Note any warnings (e.g., non-standard file type).
    4. **Your goal is to confirm readiness for deeper analysis.** If successful (hashes calculated, VT check attempted), hand off the following CRITICAL information to the Detailed Analysis Agent:
        *   The original EML `filename`.
        *   The concise `EML File Hash VT Check` summary string provided by the tool.
    5. If the file is not found, hashing fails, or the tool returns a critical error preventing basic processing, report the specific error clearly and stop the workflow.""",
...
detailed_analysis_agent = Agent[PhishingAnalysisContext](
    ...
    instructions=f"""{RECOMMENDED_PROMPT_PREFIX}
    You are the Detailed Analysis Agent, responsible for executing technical analysis tools. You receive the EML filename and its initial VT hash summary.

    YOUR TASKS (Execute Sequentially):
    1.  **Analyze Headers:** Use `analyze_eml_headers_tool` with the EML filename. Focus on the *summary* provided regarding SPF/DKIM/DMARC results and anomalies. Note if the summary indicates sender IP was not found, affecting reliability.
    2.  **Analyze Body:** Use `analyze_eml_body_tool`. Review the *summary* for any noted keywords or patterns.
    3.  **Analyze Links:** Use `analyze_eml_links_tool`. Review the *summary* covering VT/URLScan verdicts, redirects, and mismatches.
    4.  **Analyze Attachments:** Use `analyze_eml_attachments_tool`. Review the *summary* covering VT verdicts, file type checks, macro analysis, and archive inspection.

    CRITICAL INSTRUCTIONS:
    *   **Execute Accurately:** Ensure you call each tool with the correct EML `filename` obtained from the context.
    *   **Handle Errors:** If a tool returns an error message in its summary, note it. Continue to the next analysis step unless the error indicates the EML itself is unprocessable (e.g., parsing failed early).
    *   **Focus on Summaries:** Your primary output objective is to collect the *concise summary strings* generated by each analysis tool. Do NOT include raw JSON or excessive detail beyond the provided summaries.
    *   **Handoff:** Once all analysis tools have been executed, hand off *all collected summaries* (header summary, body summary, link summary, attachment summary, plus the initial EML hash summary you received) to the Final Assessment Agent for judgment. Include error summaries if any tools failed.""",
...
final_assessment_agent = Agent[PhishingAnalysisContext](
    ...
    instructions=f"""{RECOMMENDED_PROMPT_PREFIX}
    You are the Final Assessment Agent. Review ALL provided analysis summaries (EML hash, headers, body, links, attachments). Your goal is accurate classification.

    CRITICAL INSTRUCTIONS FOR EVALUATION:
    1.  **Weighting:** Do NOT treat all indicators equally.
        *   **Strong Indicators (High Weight):** Malicious verdicts from VT/URLScan (links/attachments), multiple severe auth failures (SPF+DKIM+DMARC fail/reject), links to clearly unrelated/suspicious domains, confirmed malicious macros/payloads.
        *   **Medium Indicators (Moderate Weight):** Suspicious verdicts from VT/URLScan, single auth failures (esp. DMARC fail), significant URL redirection complexity, urgent/threatening language combined with requests for action/credentials.
        *   **Weak Indicators (Low Weight - Often Benign):** Header mismatches (From/Return-Path, From/Reply-To), single DKIM failures (especially if SPF passes or sender IP unknown), generic keywords ('payment', 'account', 'invoice') without other context, simple short bodies.
        *   **Benign Signals (Strong evidence against phishing):** Clean/Benign verdicts from VT/URLScan for ALL links/attachments, SPF+DKIM+DMARC passes, known good sender reputation.
    2.  **Context is Key:** Simple keyword matches or header mismatches common in legitimate mail should NOT make an email 'Suspicious' on their own. Consider the overall picture.
    3.  **Corroboration:** Look for multiple indicators pointing the same way. An email with clean links/attachments and passing SPF should generally be considered safe, even with a DKIM issue or header mismatch.
    4.  **Reliability:** Note if summaries indicate key checks were unreliable (e.g., sender IP unknown impacting SPF/DMARC).

    YOUR TASK:
    1.  Synthesize findings, highlighting both red flags and green flags based on the weighting above.
    2.  Determine a final verdict: Malicious, Suspicious, Likely Benign, Benign, Unknown/Inconclusive. Use 'Likely Benign' if only weak indicators are present but everything else is clean.
    3.  Write a clear justification explaining HOW the combination of weighted indicators leads to your verdict. Reference specific summary points.
    4.  Use the `final_assessment_tool` with your `verdict` and `justification`.""",
        ...
        while True: # Loop until final assessment is done or an error stops the chain
            ...
            for new_item in result.new_items:
                ...
                if isinstance(new_item, MessageOutputItem):
                    ...
                    tool_name = new_item.call.tool_name
                    tool_args = new_item.call.tool_arguments
